...
2. 渐进式开关
   - 引入布尔开关控制是否启用 ACF 动态化，关闭时完全回退到当前静态 HTML，保证上线过程安全可控：  
     - 字段示例：`social_responsibility_use_acf`（真/假），挂在本页字段组根部。  

3. Fallback 策略
   - 每个字段读取时均提供静态 HTML 中已存在的默认值作为后备：  
     - 文本字段为空时，继续输出当前模板内写死的文案。  
     - 图片字段为空时，继续使用当前 `get_template_directory_uri() . '/static/image/xxx.jpg'` 的本地图片。  
...
### 1. 字段组：Social Responsibility 页面开关与 Hero

- 字段组名称：`社会责任页面 - 基础与首屏`  
- 作用范围：  
  - 位置规则：Page Template 等于 `社会责任`（即 `page-social-responsibility.php` 对应页面）  
- 字段列表：
  - `social_responsibility_use_acf`（True/False）
    - 说明：是否整体启用本页面 ACF 动态化，关闭时所有模块都采用静态 HTML。  
...
- 在 `social-responsibility-intro-and-pillars.php` 中按以下思路替换静态内容：
  - 左侧导语区：
    - `What we do` → `get_field('sr_intro_eyebrow') ?: 'What we do'`  
    - 主标题 → `get_field('sr_intro_title') ?: 'We fulfill our social obligations'`  
    - 正文部分 → 使用 `the_field('sr_intro_body_main')` 和 `the_field('sr_intro_body_sub')`，为空时保留静态段落。  
    - 按钮文字 → `get_field('sr_intro_cta_text') ?: 'CONTACT US'`，`href` 仍保持当前 Elementor Popup 链接不变，保证与其他页面共享同一弹窗。  
  - 右侧四个支柱卡片：  
    - 首选从 Repeater `sr_pillars` 渲染，按顺序输出 4 条卡片。  
    - 若 Repeater 为空或少于 4 条，则补足为静态 fallback 数据，确保布局不会破。  
...
if ($employees_image) {
    ...
    // 保留原有静态 <img>，不在这里写示例代码
}
```

- 文案：
  - 小标题：`get_field('sr_employees_eyebrow') ?: 'Responsibility for employees'`  
  - 大标题：`get_field('sr_employees_title') ?: 'We protect the physical and mental health of our employees'`  
  - 正文：若 `sr_employees_body` 有值，则输出其内容；否则继续渲染原有三段 `<p>`。  

Client / Environment / Vision 三个模块完全沿用该模式，只需替换字段前缀与默认文案。  

### 4. 全局开关的使用

- 在每个模块入口处统一判断 `social_responsibility_use_acf`：  
  - 若开关为假，直接输出当前静态 HTML（不做任何 `get_field` 调用）；  
  - 若为真，则进入上述动态化逻辑。  
- 这样可以支持“先配置完字段，在某个时间点统一切换”的运营需求。  
...
2. 先在测试环境中：
   - 为本页填充与当前静态文案一致的字段值；  
   - 开启 `social_responsibility_use_acf` 开关；  
